...
import { Camera, PhoneCall, Star, BadgeCheck, Clock, Shield } from "lucide-react";
import BookAppointmentPopup from '../Components/BookAppointmentPopup';
export default function CctvPage() {
  const cctvWorkers = [
    { name: "SecureVision CCTV", experience: "4+ Years", rating: 4.8, available: "24/7", img: "https://i.ibb.co/3MFcW0y/cctv1.png", location: "Indore" },
    { name: "WatchGuard Installers", experience: "6+ Years", rating: 4.9, available: "10AM - 10PM", img: "https://i.ibb.co/9gmG4jp/cctv2.png", location: "Ujjain" },
    { name: "HomeEye Security", experience: "5+ Years", rating: 4.7, available: "9AM - 8PM", img: "https://i.ibb.co/rmjFJR6/cctv3.png", location: "Ratlam" }
  ];
    ...
    <div className="min-h-screen w-full bg-gray-50 text-gray-800">
      ...
      <div className="bg-teal-600 text-white py-16 px-6 text-center">
        <h1 className="text-4xl font-bold mb-3 flex justify-center items-center gap-2">
          <Shield className="w-10 h-10" /> Verified CCTV Technicians
        </h1>
        <p className="text-lg max-w-2xl mx-auto opacity-90">
          CCTV Installation, Camera Repair, DVR Setup & Full Home/Office Security Solutions.
        </p>
      </div>

      {/* Filters */}
      <div className="max-w-7xl mx-auto flex gap-3 p-6 flex-wrap">
        <button className="px-4 py-2 bg-teal-600 text-white rounded-full text-sm hover:bg-yellow-600">
          ⭐ 4.5+ Rating
        </button>
        <button className="px-4 py-2 bg-teal-600 text-white rounded-full text-sm hover:bg-yellow-600">
          🕒 Fast Service
      ...
      <div className="grid md:grid-cols-3 gap-6 p-6 max-w-7xl mx-auto">
        {cctvWorkers.map((worker, index) => (
          <div key={index} className="bg-white shadow-lg p-5 rounded-2xl hover:-translate-y-1 transition text-center">
            ...
            className="mt-4 w-full flex items-center justify-center gap-2 bg-teal-600 hover:bg-yellow-600 text-white py-2 rounded-xl">
      ...
      <div className="bg-teal-600 text-white text-center py-10 mt-10">
        <h3 className="text-2xl font-semibold">Need quick surveillance setup?</h3>
        <p className="opacity-90 mt-1">We protect your home and business with best CCTV services.</p>
